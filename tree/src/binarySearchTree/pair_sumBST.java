...
public class pair_sumBST {
	...
	private ArrayList<Integer> treetolist(binarynode<Integer> root, ArrayList<Integer> list){
		if(root==null) {
			return list;
		}
	...
	public static void sum(binarynode<Integer> root, int s) {
		...
		ArrayList<Integer> a1=treetolist(root,a2);
		int start=0;
		int end=a1.size()-1;
		while(start < end) {
			if(a1.get(start)+a1.get(end)==s) {
				System.out.println(a1.get(start)+" "+ a1.get(end));
			}
			if(a1.get(start)+a1.get(end)>s) {
				end--;
			}
